    ...
    <div className="bg-[#0d1117] border border-[#1e2832] rounded-2xl overflow-hidden md:sticky md:top-20">
      ...
      <div className="flex border-b border-[#1e2832]">
        {["Insure", "My Policies"].map((tab, i) => (
          <div
            key={tab}
            className={`flex-1 py-3.5 md:py-4 text-center text-sm font-bold cursor-pointer transition-colors ${
              i === 0
            ...
            {TOKENS.map((token) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 md:gap-3 p-3 md:p-3.5 rounded-xl border text-sm font-bold transition-all ${
                    sel
        ...
        {premium && (
          <div className="bg-[#080b0f] border border-[#1e2832] rounded-xl p-4">
            ...
            <div className="flex justify-between text-sm font-bold pt-3 border-t border-[#1e2832]">
              <span className="text-[#a0b4c0] font-mono">Premium due</span>
              <span className="font-mono text-[#00e5a0] text-base">${premium} USDT</span>
            </div>
          </div>
        )}

        {/* Submit */}
        <button className="w-full bg-[#00e5a0] text-black font-mono text-sm font-bold py-3.5 md:py-4 rounded-xl hover:bg-[#00b87a] transition-colors shadow-[0_4px_20px_rgba(0,229,160,0.2)]">
          Approve & Insure →
        </button>
